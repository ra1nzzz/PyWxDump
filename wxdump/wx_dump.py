...
def gp_debug():
 ...
 if s is None or s==-1 or s==0xFFFFFFFFFFFFFFFF:return[]
 pe=E();pe.dwSize=ctypes.sizeof(pe);r=[]
...
def gp():
 ...
 if k.Process32First(s,ctypes.byref(pe)):
  while True:
   if pe.j.decode("utf-8",errors="ignore")=="WeChat.exe":r.append(pe.c)
   if not k.Process32Next(s,ctypes.byref(pe)):break
 k.CloseHandle(s);return r
def fd(pid):
 h=k.OpenProcess(P,False,pid)
 if not h or h==-1:return 0,None
 m=B();a=0
 while a<0x7FFFFFFFFFFFFFFF:
  if k.VirtualQueryEx(h,a,ctypes.byref(m),ctypes.sizeof(m))==0:break
  if m.e==0x1000 and m.d>0:
   b=ctypes.create_unicode_buffer(M)
   if p.GetMappedFileNameW(h,m.a,b,M)>0:
...
def main():
 ...
 pids=gp()
 if not pids:ap=gp_debug();print("[!] WeChat.exe not running. Found "+str(len(ap))+" procs.",file=sys.stderr);
 if not pids:
  ap=gp_debug();print("[!] WeChat.exe not running. Found "+str(len(ap))+" procs.",file=sys.stderr)
  for x in ap[:30]:print("  "+x,file=sys.stderr)
  sys.exit(1)
 print("[+] Found "+str(len(pids))+" process(es)")
